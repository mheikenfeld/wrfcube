...
#    print('loadwrfcube: loading ', variable)
...
def calculate_wrf_airmass(filenames,**kwargs):
    ...
    layer_height=derivewrfcube(filenames,'layerheight',**kwargs)
...
def calculate_wrf_pressure_stag(filenames,**kwargs):
    ...
    p_stag = 0.5*(p.extract(Constraint(bottom_top=bottom_top[:-1]))+p.extract(Constraint(bottom_top=bottom_top[1:])))    
    return p_stag
...
def calculate_wrf_pressure_xstag(filenames,**kwargs):
    ...
    p_xstag = 0.5*(p.extract(Constraint(west_east=west_east[:-1]))+p.extract(Constraint(west_east=west_east[1:])))    
...
def calculate_wrf_pressure_ystag(filenames,**kwargs):
    ...
    p_ystag = 0.5*(p.extract(Constraint(south_north=south_north[:-1]))+p.extract(Constraint(south_north=south_north[1:])))
...
def calculate_wrf_geopotential_height_ystag(filenames,**kwargs):
    ...
    z_ystag = 0.5*(z.extract(Constraint(south_north=south_north[:-1]))+z.extract(Constraint(south_north=south_north[1:])))
...
def calculate_wrf_geopotential_height_xstag(filenames,**kwargs):
    ...
    z_xstag = 0.5*(z.extract(Constraint(west_east=west_east[:-1]))+z.extract(Constraint(west_east=west_east[1:])))
...
def addcoordinates(filenames, variable,variable_cube,add_coordinates=None,slice_time=slice(None),constraint=None):
